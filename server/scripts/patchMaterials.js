...
const regex = /const content = JSON\.parse\(rows\[0\]\.content_json\);\s*res\.json\(\{ content \}\);/;
const replacement = `      let content = JSON.parse(rows[0].content_json);
      ...
      if (content && content.sections) {
        ...
        const normalizeQuestions = (obj) => {
          if (Array.isArray(obj)) {
            ...
            // Ensure gap_fill questions have prompts
