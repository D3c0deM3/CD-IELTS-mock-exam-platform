...
router.get("/:id", async (req, res) => {
  const { id } = req.params;
  try {
    const [testRows] = await db.execute("SELECT * FROM tests WHERE id = ?", [
      id,
    ]);
    ...
    const [sectionRows] = await db.execute(
      "SELECT * FROM sections WHERE test_id = ?",
      [id]
    );
...
router.post("/:id/submit", async (req, res) => {
  ...
  try {
    // Check if user is active before allowing test submission
    ...
    if (userRows[0].status !== "active") {
      return res
        ...
        .json({
          error:
            "Your account is not active. Please contact admin after paying the testing fee.",
        });
    }
